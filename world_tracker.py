...
class App:
    ...
    def refresh_watched_players(self):
        ...
        for name in self.watched_players:
            ...
            status = "ONLINE" if world else "UNKNOWN"
            world_text = str(world) if world else "—"
            last = f"{max(0, int(time.time() - seen))}s" if seen else "—"
    ...
    def set_dink_enabled(self):
        ...
        self.dink_status.set(
            "Dink enabled • watched players only"
            if self.dink_enabled.get()
            else "Dink input disabled"
        )
    ...
    def build_ui(self):
        ...
        ttk.Label(
            ...
            text=(
                ...
                "Dink is experimental; non-watched events\n"
                "are ignored."
            ),
            foreground="#8d9ab0",
            justify="left"
