...
class GroupChain:
    ...
    def size(self):
        recent = self.hops[-5:]
        return round(
            sum(h.moved for h in recent) / len(recent)
        )
    ...
    def can_extend(self, hop):
        ...
        if abs(hop.moved - self.size) / self.size > 0.35:
            return False

        return (
            hop.timestamp - self.last_time <= 45
        )
